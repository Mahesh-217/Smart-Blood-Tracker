        ...
        FirebaseDatabase.getInstance().getReference("Donors").addValueEventListener(new ValueEventListener() {
            ...
            public void onDataChange(@NonNull DataSnapshot snapshot) {
                ...
                for (DataSnapshot ds : snapshot.getChildren()) {
                    ...
                    if (user != null && user.getStep() != null && user.getVisible() != null && user.getUID() != null) {
                        if (user.getStep().equals("Done")) {
                            if ("True".equals(user.getVisible())) { // Use "True".equals() to avoid NPE
                                users.add(user);
                                temp.add(user);
                            }
                            if (uid.equals(user.getUID())) {
                                self = user;
                                popupMenu.getMenu().findItem(R.id.visibleDonors).setChecked("True".equals(self.getVisible())); // Use "True".equals() to avoid NPE
                            }
                        }
                    }
                }
